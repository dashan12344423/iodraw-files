...
graph TD;
    ...
    subgraph "车间调拨流程"
        ...
        subgraph "提交调拨阶段一（调拨中）"
            ...
            UpdateCarAllotOut -->|更改调出库存总览| UpdateInventoryOverviewOut("更改库存(调出的仓库)(<br>对应仓库，物料有则减掉，如果减完后数量<=0，则删除掉)")
        ...
        subgraph "提交调拨阶段二（已完成）"
            ...
            CalculateTotalIn -->|更改调入库存总览| UpdateInventoryOverviewIn("更改库存(调入的仓库) (<br>对应仓库，物料有则累加，没有则新增)")
