...
graph TD;
    ...
    subgraph "车间调拨流程"
        ...
        subgraph "选择物料阶段"
            StorageCarAllotCreate -->|选择物料| ChoiceTransferMaterial("选择-调拨物料")
            ChoiceTransferMaterial -->|后端操作| ChoiceTransferMaterialBackend("如果选择的调出仓库类型为线边库，则物料数据从 storage_wireline_storage 线边库明细表中拉取物料信息.<br>反之，物料数据从库存明细表 storage_inventory_detail 中获取数据.")
        end

        subgraph "保存操作阶段"
            StorageCarAllotCreate -->|保存数据| SaveNew("保存-新增")
            SaveNew -->|后端处理| SaveNewBackend("有事务回滚<br>向车间调拨主表 storage_transfer_order 表中写入数据.<br>向车间调拨明细表 storage_car_allot 中写入数据")
            SaveNew -->|日志记录| SaveNewLog("系统模块名：【车间调拨】，操作类型：【新增】。")
            StorageCarAllotCreate -->|编辑数据| SaveEdit("保存-编辑")
            SaveEdit -->|后端处理| SaveEditBackend("有事务回滚<br>通过调拨单号更新车间调拨主表 storage_transfer_order 中数据<br>通过调拨单号先删除车间调拨明细表 storage_car_allot 中数据,然后再向车间调拨明细表 storage_car_allot 中写入数据")
            SaveEdit -->|日志记录| SaveEditLog("系统模块名：【车间调拨】，操作类型：【编辑】。")
        end

        subgraph "完成操作阶段"
            StorageCarAllotCreate -->|标记完成| Completed("已完成")
            Completed -->|后端更新| CompletedBackend("通过调拨单编号更新车间调拨单主表 storage_transfer_order 中数据状态 allot_status=2.已完成")
            Completed -->|日志记录| CompletedLog("系统模块名：【车间调拨】，操作类型：【已完成】。")
        end

        subgraph "调拨单号生成阶段"
            StorageCarAllotCreate -->|生成单号| TransferOrderNo("【车间调拨单号】")
            // 这里可根据实际需要进一步细化单号生成逻辑，目前仅保留环节示意
        end
    end
