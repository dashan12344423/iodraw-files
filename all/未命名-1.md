...
graph TD;
    ...
    subgraph "车间调拨流程"
        ...
        subgraph "选择调拨物料子流程"
            StorageCarAllotCreate -->|选择调出仓库| SelectOutWarehouse("选择调出仓库")
            SelectOutWarehouse -->|判断仓库类型| CheckWarehouseType("判断仓库类型是否为线边库")
            CheckWarehouseType -->|是线边库| GetWirelineData("从 storage_wireline_storage 线边库明细表中拉取物料信息")
            CheckWarehouseType -->|非线边库| GetInventoryData("从库存明细表 storage_inventory_detail 中获取数据")
            GetWirelineData -->|传递物料信息| PassWirelineMaterial("将获取的线边库物料信息传递下一步")
            GetInventoryData -->|传递物料信息| PassInventoryMaterial("将获取的库存明细物料信息传递下一步")
            PassWirelineMaterial -->|用于后续操作| StorageCarAllotCreate
            PassInventoryMaterial -->|用于后续操作| StorageCarAllotCreate
        end
    end
