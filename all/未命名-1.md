...
graph TD;
    ...
    subgraph "车间调拨流程"
        ...
        subgraph "提交调拨阶段"
            StorageCarAllotDetailCreate -->|提交操作，先进入调拨中状态| Submitaa("提交")
            Submitaa -->|更新调拨单状态为调拨中，同时处理相关数据| UpdateTransferOrderMid("根据调拨单号将 storage_transfer_order 表中入库状态 allot_status 改为调拨中，<br>填写实际调拨人、调拨日期等")
